# -*- coding: utf-8 -*-
"""
Created on Mon Aug 14 15:26:14 2023
@author: XCH
"""
...
class CNN(nn.Module):
    def __init__(self, configs):
        ...
        # input_channels=2, mid_channels=32, kernel_size=3, stride=1
        ...
        # mid_channels=32, kernel_size=3, stride=1
        self.conv_block2 = nn.Sequential(
            nn.Conv1d(configs.mid_channels, configs.mid_channels * 2, kernel_size=configs.kernel_size, 
                      stride=configs.stride, bias=False, padding=(configs.kernel_size//2)),
        ...
        self.conv_block3 = nn.Sequential(
            nn.Conv1d(configs.mid_channels * 2, configs.final_out_channels, kernel_size=configs.kernel_size, 
                      stride=configs.stride, bias=False, padding=(configs.kernel_size//2)),
    ...
    def forward(self, x_in):
        x = self.conv_block1(x_in) # (32, 32, 19)
        x = self.conv_block2(x)    # (32, 64, 10)
        x = self.conv_block3(x)    # (32, 128, 6)
        x = self.adaptive_pool(x)  # (32, 128, 1)
...
class LSTM(nn.Module):
    def __init__(self, configs):
        ...
        self.lstm = nn.LSTM(configs.input_channels, configs.lstm_hid, num_layers=configs.lstm_layers, bias=configs.lstm_bid, dropout=configs.dropout)
        self.relu = nn.ReLU()
